    ...
    public void showSnackBar(FragmentActivity activity, CharSequence message1, int length, CharSequence message2, View.OnClickListener listener, float textSize, int color) {
        if (activity != null) {
            ...
            if (textSize != Constant.View.DEFAULT_SIZE) {
                ((TextView) ViewUtil.getInstance().findView(snackbar.getView(), com.dasset.wallet.components.R.id.snackbar_text)).setTextSize(textSize);
            }
            if (color != Constant.View.DEFAULT_COLOR) {
                ((TextView) ViewUtil.getInstance().findView(snackbar.getView(), com.dasset.wallet.components.R.id.snackbar_text)).setTextColor(color);
            }
            snackbar.show();
        }
    }
