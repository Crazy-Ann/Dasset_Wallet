package com.dasset.wallet.components.http.request;
...
import com.dasset.wallet.components.http.HttpCallUtil;
import com.dasset.wallet.components.http.HttpTask;
...
public final class HttpRequest {
    ...
    private void executeRequest(Context ctx, HttpRequestType httpRequestType, String url, RequestParameter requestParameter, OkHttpClient.Builder builder, HttpResponse httpResponse) {
        if (NetworkUtil.getInstance().isInternetConnecting(ctx)) {
            ...
            ToastUtil.getInstance().showToast(ctx, ctx.getString(com.dasset.wallet.components.R.string.http_error_prompt1), Toast.LENGTH_SHORT);
        }
    }
