...
public class Bip39 {
    ...
    public static class MasterSeed implements Serializable {
        ...
        private final byte[] bip39RawEntropy;
        private final String bip39Passphrase;
        private final byte[] bip32MasterSeed;
        private final byte _wordListType;

        @SuppressWarnings("NewApi")
        private MasterSeed(byte[] bip39RawEntropy, String bip39Passphrase, byte[] bip32MasterSeed) {
            this.bip39RawEntropy = bip39RawEntropy;
            this.bip39Passphrase = Normalizer.normalize(bip39Passphrase, Normalizer.Form.NFKD);
            this.bip32MasterSeed = bip32MasterSeed;
            _wordListType = ENGLISH_WORD_LIST_TYPE;
        }

        public List<String> getBip39WordList() {
            return Arrays.asList(rawEntropyToWords(bip39RawEntropy));
        }
        ...
        public byte[] getBip32Seed() {
            return BitUtils.copyByteArray(bip32MasterSeed);
        }
        ...
        public byte[] toBytes(boolean compressed) {
            ...
            // Add the raw entropy
            putByteArray(bip39RawEntropy, writer);

            // Add the passphrase
            try {
                putByteArray(bip39Passphrase.getBytes("UTF-8"), writer);
            ...
            if (!compressed) {
                putByteArray(bip32MasterSeed, writer);
            }
        ...
        private static byte[] getByteArray(ByteReader reader) throws InsufficientBytesException {
        ...
        @Override
        public int hashCode() {
            return (int) BitUtils.uint32ToLong(bip32MasterSeed, 0);
        }
        ...
        public boolean equals(Object obj) {
            ...
            return BitUtils.areEqual(bip32MasterSeed, other.bip32MasterSeed);
        }
    }
    ...
    public static MasterSeed generateSeedFromWordList(List<String> wordList, String passphrase) {
        ...
        byte[] seed;
        try {
            byte[] saltBytes = Normalizer.normalize(salt, Normalizer.Form.NFKD).getBytes(Regex.UTF_8.getRegext());
            seed = PBKDF.pbkdf2(ALGORITHM, mnemonic.getBytes(Regex.UTF_8.getRegext()), saltBytes, REPETITIONS, BIP32_SEED_LENGTH);
