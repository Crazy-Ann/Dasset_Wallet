...
public class Address implements Serializable, Comparable<Address> {
    ...
    public static Collection<Address> fromStrings(Collection<String> addresses, NetworkParameters networkParameters) throws AddressFormatException {
        List<Address> list = new LinkedList<Address>();
        for (String address : addresses) {
            Address a = Address.fromString(address, networkParameters);
            if (a == null) {
                return null;
            }
            list.add(a);
    ...
    public static Address[] fromStrings(String[] data) throws AddressFormatException {
        Address[] addresses = new Address[data.length];
        for (int i = 0; i < data.length; i++) {
            addresses[i] = Address.fromString(data[i]);
        }
        return addresses;
    }
    ...
    public static String[] toStrings(Address[] addresses) {
        ...
        for (int i = 0; i < data.length; i++) {
            data[i] = addresses[i].toString();
        }
        return data;
    }

    public static Address fromString(String data, NetworkParameters networkParameters) throws AddressFormatException {
        Address address = Address.fromString(data);
        if (address == null) {
            return null;
        }
        if (!address.isValidAddress(networkParameters)) {
            return null;
        }
        return address;
    }
    ...
    /**
     * @param data string representation of an address
     *
     * @return an Address if address could be decoded with valid checksum and length of 21 bytes
     * null else
     */
    public static Address fromString(String data) throws AddressFormatException {
        if (data == null) {
            return null;
        }
        if (data.length() == 0) {
        ...
        byte[] bytes = Base58.decodeChecked(data);
