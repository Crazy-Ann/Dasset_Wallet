...
public final class ECKeystoreFactory {


    private static ECKeystoreFactory eCSignatureFactory;
    ...
    private ECKeystoreFactory() {
        // cannot be instantiated
    }

    public static synchronized ECKeystoreFactory getInstance() {
        if (eCSignatureFactory == null) {
            eCSignatureFactory = new ECKeystoreFactory();
        }
        return eCSignatureFactory;
    }

    public static void releaseInstance() {
        if (eCSignatureFactory != null) {
            eCSignatureFactory = null;
        }
...
//            Sha256Hash sha256Hash = Sha256Hash.create(Utils.formatMessageForSigning(data));
    ...
    public byte[] generateSignature(BigInteger privateKey, byte[] publicKey, String data, boolean compressed, boolean isRandom) throws KeyCrypterException, InvalidAlgorithmParameterException, NoSuchAlgorithmException, NoSuchProviderException {
        if (privateKey != null) {
            ...
            int recoverId = -1;
            for (int i = 0; i < 4; i++) {
                ECKeyPairFactory ecKeyPairFactory = recoverFromSignature(i, ecdsaSignature, Sha256Hash.create(data.getBytes()), compressed);
                if (ecKeyPairFactory != null) {
                    LogUtil.getInstance().print(String.format("Public Key[%s]: %s", i + 1, Hex.toHexString(ecKeyPairFactory.getPublicKey())));
                    LogUtil.getInstance().print(String.format("Public Key: %s", Hex.toHexString(publicKey)));
                    if (Arrays.equals(ecKeyPairFactory.getPublicKey(), publicKey)) {
                        recoverId = i;
                        break;
                    }
                }
    ...
    private ECKeyPairFactory generateSignature(String data, String signature) throws SignatureException, InvalidAlgorithmParameterException, NoSuchAlgorithmException, NoSuchProviderException {
...
//        Sha256Hash sha256Hash = Sha256Hash.createDouble(Utils.formatMessageForSigning(data));
        ...
        ECKeyPairFactory ecKeyPairFactory = recoverFromSignature(recoverId, ecdsaSignature, sha256Hash, compressed);
        if (ecKeyPairFactory == null) {
            throw new SignatureException("Could not recover public key from signature");
        }
        return ecKeyPairFactory;
    }
    ...
    public boolean verifySignature(String data, String signature, byte[] publicKey) throws SignatureException, InvalidAlgorithmParameterException, NoSuchAlgorithmException, NoSuchProviderException {
        ECKeyPairFactory ecKeyPairFactory = generateSignature(data, signature);
        return ecKeyPairFactory != null && Arrays.equals(ecKeyPairFactory.getPublicKey(), publicKey);
    }

    private ECKeyPairFactory recoverFromSignature(int recoverId, ECKey.ECDSASignature ecdsaSignature, Sha256Hash sha256Hash, boolean compressed) throws InvalidAlgorithmParameterException, NoSuchAlgorithmException, NoSuchProviderException {
        ...
        if (x.compareTo(prime) >= 0) {
            ...
            if (!R.multiply(n).isInfinity()) {
                ...
                LogUtil.getInstance().print(String.format("Public Key: %s", Hex.toHexString(publicKey)));
                return new ECKeyPairFactory(null, publicKey, compressed);
            }
        }
    }
