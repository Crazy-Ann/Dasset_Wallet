...
public class SecureKeyValueStore {
    ...
    protected final SecureKeyValueStoreBacking secureKeyValueStoreBacking;
    ...
    public SecureKeyValueStore(SecureKeyValueStoreBacking backing, RandomSource randomSource) {
        secureKeyValueStoreBacking = backing;
    ...
    public void deletePlaintextValue(byte[] id) {
        secureKeyValueStoreBacking.deleteValue(getRealId(id, false));
    }
    ...
    public boolean isValidEncryptionKey(KeyCipher userCipher) {
        try {
            ...
            return true;
        } catch (InvalidKeyCipherException e) {
            return false;
        }
    }
     ...
     */
    public synchronized void replaceEncryptionKey(KeyCipher currentUserCipher, KeyCipher newUserCipher) throws InvalidKeyCipherException {
     ...
     *
     * @throws InvalidKeyCipherException if the specified encryption key is invalid
     */
    public synchronized byte[] getEncryptedValue(byte[] id, KeyCipher userCipher) throws InvalidKeyCipherException {
        if (id.length == 0) {
            throw new RuntimeException("IDs cannot have zero length");
        }
     ...
     * @param userCipher     the user defined encryption key
     *
     * @throws InvalidKeyCipherException if the user defined encryption key is invalid
     */
    public synchronized void encryptAndStoreValue(byte[] id, byte[] plaintextValue, KeyCipher userCipher) throws InvalidKeyCipherException {
        if (id.length == 0) {
            throw new RuntimeException("IDs cannot have zero length");
        }
        AesKeyCipher kekCipher = getKeyEncryptionKey(userCipher); // may throw InvalidKeyCipher
    ...
    public void deleteEncryptedValue(byte[] id, KeyCipher userCipher) throws InvalidKeyCipherException {
        AesKeyCipher kekCipher = getKeyEncryptionKey(userCipher); // may throw InvalidKeyCipher
        secureKeyValueStoreBacking.deleteValue(getRealId(id, true));
    }

    private synchronized AesKeyCipher getKeyEncryptionKey(KeyCipher userCipher) throws InvalidKeyCipherException {
    ...
    private byte[] getEncryptedKeyEncryptionKey() {
        ...
        return secureKeyValueStoreBacking.getValue(KEK_ID);
    }
    ...
    private void storeEncryptedKeyEncryptionKey(byte[] encryptedKek) {
        ...
        // from a SecureSubKeyValueStore
        secureKeyValueStoreBacking.setValue(KEK_ID, encryptedKek);
    }
    ...
    public SecureSubKeyValueStore getSubKeyStore(int forSubId) {
        return new SecureSubKeyValueStore(secureKeyValueStoreBacking, randomSource, forSubId);
    }

    public synchronized SecureSubKeyValueStore createNewSubKeyStore() {
        int maxSubId = secureKeyValueStoreBacking.getMaxSubId();

        SecureSubKeyValueStore subKeyValueStore = new SecureSubKeyValueStore(secureKeyValueStoreBacking, randomSource, maxSubId + 1);
    ...
    protected synchronized byte[] getValue(byte[] realId) {
        return secureKeyValueStoreBacking.getValue(realId);
    }

    protected synchronized void setValue(byte[] realId, byte[] value) {
        secureKeyValueStoreBacking.setValue(realId, value);
    }
}
