...
public class SignedMessage implements Serializable {
    ...
    public static PublicKey recoverFromSignature(String message, String signature) throws SignatureException {
        return recoverFromSignature(message, Base64.decode(signature), decodeSignature(Base64.decode(signature))).publicKey;
    }

    public static SignedMessage validate(Address address, String message, String signatureBase64) throws SignatureException {
        final byte[] signatureEncoded = Base64.decode(signatureBase64);
        if (signatureEncoded == null) {
            // Invalid or truncated base64
            throw new SignatureException(String.format("given signature is not valid base64 %s", signatureBase64));
        }
    ...
    public static void validateAddressMatches(Address address, PublicKey key) throws SignatureException {
        Address recoveredAddress = key.toAddress(address.getNetwork());
        if (!address.equals(recoveredAddress)) {
            throw new SignatureException(String.format("given Address did not match \nexpected %s\n but got %s", address, recoveredAddress));
        }
    }
     ...
     */
    private static RecoveryInfo recoverFromSignature(String message, byte[] signatureEncoded, Signature sig) throws SignatureException {
    ...
    private static Signature decodeSignature(byte[] signatureEncoded) throws SignatureException {
