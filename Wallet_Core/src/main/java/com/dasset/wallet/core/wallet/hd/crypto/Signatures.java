...
public class Signatures {
    ...
    public static byte[][] decodeSignatureParameterBytes(ByteReader byteReader) {
        try {
            ...
            if ((((int) byteReader.get()) & 0xFF) != 0x30) {
                return null;
            }

            // Read total length as a byte, standard inputs never get longer than
            // this
            int length = ((int) byteReader.get()) & 0xFF;

            // Read first type, must be 0x02
            if ((((int) byteReader.get()) & 0xFF) != 0x02) {
                return null;
            }

            // Read first length
            int length1 = ((int) byteReader.get()) & 0xFF;

            // Read first byte array
            byte[] bytes1 = byteReader.getBytes(length1);
            ...
            // Read second length
            int length2 = ((int) byteReader.get()) & 0xFF;

            // Read second byte array
            byte[] bytes2 = byteReader.getBytes(length2);
