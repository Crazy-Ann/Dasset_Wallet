...
import com.dasset.wallet.core.random.camera.CameraManager;

import java.io.IOException;

public class UEntropyCamera implements SurfaceHolder.Callback, IUEntropySource, Thread.UncaughtExceptionHandler {
    ...
    public UEntropyCamera(SurfaceView surfaceView, UEntropyCollector uEntropyCollector) {
    ...
    private final Runnable openRunnable = new Runnable() {
        ...
        public void run() {
            try {
                ...
                final boolean nonContinuousAutoFocus = Camera.Parameters.FOCUS_MODE_AUTO.equals(focusMode) || Camera.Parameters.FOCUS_MODE_MACRO.equals(focusMode);
                if (nonContinuousAutoFocus) {
                    handler.post(new AutoFocusRunnable(camera));
                }
                handler.post(fetchCameraDataRunnable);
            } catch (IOException e) {
        ...
        public void run() {
            camera.autoFocus(new Camera.AutoFocusCallback() {
                ...
                public void onAutoFocus(final boolean success, final Camera camera) {
                    // schedule again
                    handler.postDelayed(AutoFocusRunnable.this, AUTO_FOCUS_INTERVAL_MS);
                }
            });
        }
    }
        ...
        public void run() {
            cameraManager.requestPreviewFrame(new Camera.PreviewCallback() {
                ...
                public void onPreviewFrame(final byte[] data, final Camera camera) {
                    uEntropyCollector.onNewData(data, UEntropyCollector.UEntropySource.Camera);
                    handler.post(fetchCameraDataRunnable);
                }
            });
        }
    ...
    public void uncaughtException(Thread thread, Throwable ex) {
        uEntropyCollector.onError(new Exception(ex), UEntropyCamera.this);
    }
